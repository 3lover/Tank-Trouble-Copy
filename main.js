...
function startScreen() {
    ...
    moveIn(WIDTH + 10,
        [-WIDTH,
            0 + HEIGHT / 45,
            WIDTH - 20,
            HEIGHT * 0.2 - HEIGHT / 15, "mainMenuTitle"
        ],
    ...
    moveIn(WIDTH + 10,
        [-WIDTH,
            ...
            WIDTH - 20,
            HEIGHT / 4, "startingOptionsSelector"
        ],
...
function options() {
    ...
    let weaponOptions = {
        ...
        length = Object.keys(weaponOptions)
        .length;
    moveIn(WIDTH + 10,
        [-WIDTH,
            0 + HEIGHT / 45,
            WIDTH - 20 - WIDTH / 5,
            HEIGHT * 0.2 - HEIGHT / 15, "optionsTitle"
        ],
        ["Settings:", HEIGHT / 45, 0, "start", WIDTH / 30], 1);
    moveIn(-WIDTH + WIDTH - WIDTH / 5,
        [
            WIDTH,
            0 + HEIGHT / 45,
            WIDTH / 5 - 10,
            HEIGHT * 0.2 - HEIGHT / 15, "optionsBack"
        ],
...
class Entity {
    constructor(id, image, PROPS = {}, type) {
        ...
        this.collisions = function() {
            if (this.x + this.width / 2 > WIDTH || this.x - this.width / 2 < 0 || this.y + this.height * 2 > HEIGHT || this.y - this.height / 2 < 0) return "w";
            for (let j = 0; j < entities.length; j++) {
                ...
                if (dx * dx + dy * dy <= ((obj.width / 2) ** 2)) return "w";
            }
            return null;
        }
        ...
        this.refresh = function() {
            ...
            switch (type) {
                case 5:
                    ...
                    break;
                case 4:
                    this.text = eval(CONTENT[0])
                        .toString();
                    break;
                case 3:
                    this.options = (this.text.match(/#b/g) || [])
                        .length + 1;
            ...
            this.clicked = function(event) {
                ...
                switch (this.id) {
                    case "optionsBack":
                        ...
                        break;
                    case "gamespeedBtn":
                        gameInfo.gamespeed = gameInfo.gamespeed >= 3 ? 0.25 : gameInfo.gamespeed < 1 ? gameInfo.gamespeed + 0.25 : gameInfo.gamespeed + 0.5;
                        refresh();
                        break;
                    case "enableW":
            ...
            if (type == 2 || type == 1 || type == 4) {
                ...
                for (let i = 0; i < stringLines.length; i++) {
                    ...
                    ctx.fillText(stringLines[i], this.x + CONTENT[1], this.y + CONTENT[2] + i * CONTENT[4] + this.height / 2);
                    if (i > 2) break;
                }
            }
            ctx.beginPath();
...
function getMousePosition(event) {
    ...
    for (let i = 0; i < entities.length; i++)
        if (insideRect(event.clientX, event.clientY, entities[i])) entities[i].clicked(event);
}

function numberSubmit(num) {
    let select;
    for (let i = 0; i < entities.length; i++)
        if (entities[i].type == 3 && entities[i].options > num) select = entities[i];
